...
import com.alura.latam.modelo.CalcularDias;
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtFechaEntrada.getDate() != null && ReservasView.txtFechaSalida.getDate() != null) {		
					...
					boolean errorRegistrado = true;
	
					LocalDate localFechaEntrada =  fechaEntrada.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
					LocalDate localFechaSalida =  fechaSalida.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

					 long diasDiferencia = ChronoUnit.DAYS.between(localFechaEntrada, localFechaSalida);
					 
					 int diasTomados = (int) diasDiferencia;
					 
					 
					 if (diasTomados < 1) {
						 JOptionPane.showMessageDialog(null, "La Reserva debe ser por lo menos de 1.");
						 errorRegistrado = false;
					 } else if (diasTomados == 1 && diasTomados < 3 ) {
						 valor = diasTomados * 300;
					 }else if (diasTomados == 4 && diasTomados < 7) {
						 valor = diasTomados * 285;
					 }else {
						 valor = diasTomados * 275;
					 }
					 
					 
					 try {
						 if(errorRegistrado) {
							CalcularDias diferencia = new CalcularDias();
							RegistroHuesped registro = new RegistroHuesped();	 
							diferencia.diferenciaDias(fechaEntrada, fechaSalida, valor, formaPago);
